...
const numeroClasif: number = numero2 === 0 ? 0 : numero2 > 2 ? 1 : 2
...
const nombres: string[] = ["Juan", "Pedro", "Pablo"]
...
const resultSum: number = numeros.reduce((suma, valor) => suma + valor)
...
const nums: number[] = [11, 12, 3, 34, 51, 62, 27, 58, 19, 1]
...
class carAdministrator {
    ...
    static showInformation(car: ICar):void {
        console.log("\tInformacion del vehiculo:")
        console.log(`\tMarca: ${car.marca}\n\tModelo: ${car.modelo}\n\tAño: ${car.año}`)
    }

    static increaseYear(car:ICar): ICar{
        const newCar: ICar = {
            ...car,
            año: car.año + 1
        }
...
const car: ICar = { marca: "Chevrolet", modelo: "Camaro", año: 2006 }
...
const nums2: number[] = [11, 12, 3, 34, 51, 62, 27, 58, 19, 1]
const maxNumber = (arr: number[]): number => arr.reduce((max, current) => (current > max ? current : max));
...
function validateParams(...params: (string|number|boolean)[]): boolean {
